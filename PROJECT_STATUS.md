...
Notomattic v2 is a Tauri + React note-taking app for macOS. The core note-taking functionality is implemented and working. The app can create, read, update, and organize daily notes with a rich text editor.

**Stack:** TypeScript/React (Frontend) + Rust/Tauri (Backend)
**Storage:** `~/Documents/Notomattic/` (filesystem-based)
...
| **No input validation** | Special chars break filenames | `Sidebar.tsx`, `lib.rs` |
| **No user error messages** | Users don't know when ops fail | All hooks |

### Known Problems

1. **HTML stored as .md files**
   - Files have `.md` extension but contain HTML from TipTap
   - Direct file editing shows raw HTML markup
   - Should convert to Markdown or use different extension

2. **No delete UI**
   - `delete_note` command exists in Rust backend
   - No button or menu option in frontend
   - Notes can only be deleted manually from filesystem

3. **Timeline is placeholder**
...
5. **Potential path traversal**
   - Note titles aren't sanitized
   - Characters like `../` could create files outside app folder
...
1. **Delete Notes UI** - Add delete button to editor header
2. **Search** - Find notes by title (at minimum)
3. **Error Notifications** - Show user when operations fail
4. **Input Validation** - Sanitize note titles
