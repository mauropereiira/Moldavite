...
- **Local semantic search.** Opt-in, fully-local "search by meaning" for your notes. Enable it in **Settings → AI & Agents**: after an explicit consent prompt, Moldavite downloads a small AI model (all-MiniLM-L6-v2, ~97 MB) once from HuggingFace — cached in the app data dir, never inside a vault — and builds a per-Forge vector index with live download/indexing progress. From then on everything runs fully offline; your notes never leave your Mac (locked notes are never read or indexed). Once the index is ready: the sidebar search gains a **Keyword / Semantic** mode chip that finds notes by meaning rather than exact words, and a **Related** section below the editor (next to Linked mentions) lists the 5 most similar notes to the one you're reading. Notes are re-embedded incrementally on save/delete/rename/trash/restore, and a "Rebuild index" button in Settings re-embeds everything from scratch.
