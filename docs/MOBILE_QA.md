...
- Index title/search and the pages keep a 16pt inset beside the 44pt rail on
  either side (with the rail on the left they used to start 6pt from it, level
  with the clock, which read cramped). Other device sizes and rotation still
  need verification.
