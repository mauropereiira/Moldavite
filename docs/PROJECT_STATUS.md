...
- Quick switcher / command palette (⌘P on macOS, Ctrl+P on Windows and Linux), backend full-text search with snippets answered from a per-Forge SQLite FTS5 index kept in the app data directory (v2.6: 10,000 notes answer in about 2 ms against 222 ms for the old scan, 50,000 in 11 ms against 1.3 s; the scan remains the fallback; Settings → AI & Agents shows the index state and can rebuild it), timeline view; opening any note yields transient Timeline/Graph views so navigation cannot remain hidden behind them
